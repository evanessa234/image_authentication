...
import os
import LogisticDecryption as Ld
...
def performEntireDecryption():
    filename = entry1.get()
    key = entry2.get()
    resImage1 = hlD.pixelManipulation2(filename, key)
...
def performLogisticManipulation():
    filename = entry1.get()
    key = entry2.get()
    resImage = Ld.LogisticDecryption(filename, key)
    entry4.insert(0,resImage)
    #print(filename)

def performHenonManipulation():
    filename = entry1.get()
    resImage = hD.HenonDecryption(filename)
...
button2 = Button(Frame3, text = "Perform Decryption",command = performEntireDecryption,width=20)
