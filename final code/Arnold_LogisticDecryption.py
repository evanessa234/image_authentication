...
def inverseArnold(img):
    img = cv2.imread(img)
    #for i in range(0,key):
    orig_img = np.copy(img)
    img = transform(img)
    
    cv2.imwrite('./processed_images/ArnoldDecryptedWatermark.jpg',img)
    absPath = os.path.abspath("./processed_images/ArnoldDecryptedWatermark.jpg")
    return absPath


def pixelManipulation1(imageNameEnc):
    ...
    absPath = os.path.abspath(imageNameEnc.split('.')[0] + "_HenonDec.png")
    return absPath
...
def pixelManipulation2(imageName, key):
    ...
    absPath = os.path.abspath(imageName.split('_')[0] + "_LogisticDec.png")
    return absPath