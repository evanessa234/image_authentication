...
def arnold(path):
    ...
    img = transform(img)
        #cv2.imwrite('./processed_images/scrambledWatermark' + str(i) + '.jpg',img)
    cv2.imwrite('./processed_images/scrambledWatermark.jpg',img)
    absPath = os.path.abspath("./processed_images/scrambledWatermark.jpg")
    return absPath

def random_key():
    N = 12
    res = ''.join(random.choices('0123456789' + 'abcdef', k = N))
    return res
