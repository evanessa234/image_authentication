...
def DWT_GRAY_HL_EXTRACT(coverImagePath, watermarkImagePath, watermarked_img):
    ...
    extracted_watermark, watermarked_band = extract_gray_watermark_HL(cover_HL, watermarked_HL)
    cover = pywt.idwt2((watermarked_LL, (watermarked_LH, watermarked_band, watermarked_HH)), 'haar')
    
    #extracted_cover = np.uint8(extracted_cover)

    extracted_watermark = pywt.idwt2((watermark_LL, (watermark_LH, extracted_watermark, watermark_HH)), 'haar')
    watermark = pywt.idwt2((watermarked_LL, (watermarked_LH, watermarked_HL, watermarked_HH)), 'haar')
    #extracted_cover = extract_gray_cover_HL(extracted_watermark, watermark)
    
    extracted_watermark = np.uint8(extracted_watermark)
    

    out_path1 = IMAGES_DIR + 'extracted_watermark_DWT_GRAY_HL.jpg'
    cv2.imwrite(out_path1, extracted_watermark)
    
    
    out_path2 = IMAGES_DIR + 'extracted_coverimage_DWT_GRAY_HL.jpg'
    cv2.imwrite(out_path2, cover)
...
def extract_gray_watermark_HL(cover_band, watermarked_band):
    ...
    return extracted_watermark, watermarked_band

#def extract_gray_cover_HL(extracted_watermark, watermarked_band):
#    cover = (watermarked_band - extracted_watermark) / (0.6)
 #   return cover
